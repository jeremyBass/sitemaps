...
class Wsu_Adminhtml_Block_System_Config_Form_Fieldset_Wsu_Support
	extends Wsu_Adminhtml_Block_System_Config_Form_Fieldset_Wsu_Abstract
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
		...
		$fields = array(
            ...
            array('type' => 'label', 'name' => 'send', 'after_element_html' => '<div class="right"><button type="button" class="scalable save" onclick="wsuSupport();">'.$this->__('Send').'</button></div><div class="notice" id="ajax-response"></div>'),
  		);
    ...
    {
        $ajaxUrl = $this->getUrl('wsu/support');
        $html = parent::_getFooterHtml($element);
        $html = '<h4>'.$this->__('Contact Wsu Support Team or visit <a href="%s">%s</a> for additional information', 'http://www.wsu.com/', 'Wsu.com').'</h4>' . $html;
