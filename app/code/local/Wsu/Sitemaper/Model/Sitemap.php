 ...
 * @author     Wsu Dev Team
 */
class Wsu_Sitemaper_Model_Sitemap extends Mage_Core_Model_Abstract {
    ...
    protected function _beforeSave() {
        ...
        if (!$io->allowedPath($realPath, Mage::getBaseDir())) {
            Mage::throwException(Mage::helper('sitemaper')->__('Please define correct path'));
        }

        if (!$io->fileExists($realPath, false)) {
            Mage::throwException(Mage::helper('sitemaper')->__('Please create the specified folder "%s" before saving the sitemap.', $this->getSitemapPath()));
        }

        if (!$io->isWriteable($realPath)) {
            Mage::throwException(Mage::helper('sitemaper')->__('Please make sure that "%s" is writable by web-server.', $this->getSitemapPath()));
        }

        if (!preg_match('#^[a-zA-Z0-9_\.]+$#', $this->getSitemapFilename())) {
            Mage::throwException(Mage::helper('sitemaper')->__('Please use only letters (a-z or A-Z), numbers (0-9) or underscore (_) in the filename. No spaces or other characters are allowed.'));
        }
    ...
    public function generateXml($entity=false) {
        $this->_useIndex = Mage::getStoreConfigFlag('wsu_seo/google_sitemap/use_index');
        $this->_splitSize = (int) Mage::getStoreConfig('wsu_seo/google_sitemap/split_size') * 1024;
        $this->_maxLinks = (int) Mage::getStoreConfig('wsu_seo/google_sitemap/max_links');
        ...
        if (!$entity || $entity=='category') {
            $changefreq = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/category_changefreq');
            $priority = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/category_priority');
            ...
            // categories
            $collection = Mage::getResourceModel('sitemaper/catalog_category')->getCollection($storeId);
        ...
        if (!$entity || $entity=='product') {
            $productImages = Mage::getStoreConfigFlag('wsu_seo/google_sitemap/product_images');
            $imagesSize = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/product_images_size');
            if (!preg_match('/^\d+x\d+$/', $imagesSize)) {
                $imagesSize = false;
            }
            $changefreq = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/product_changefreq');
            $priority = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/product_priority');
            
            
            $this->_totalProducts = Mage::getResourceModel('sitemaper/catalog_product')->getCollection($this->getStoreId(), true);
            if ($this->_totalProducts>0) {
                if ($entity) {
                    $limit = 500;
                    if ($this->_currentInc<$this->_totalProducts) {
                        $collection = Mage::getResourceModel('sitemaper/catalog_product')->getCollection($storeId, false, $limit, $this->_currentInc);
                    ...
                    $collection = Mage::getResourceModel('sitemaper/catalog_product')->getCollection($storeId);
                }    
                ...
                foreach ($collection as $item) {
                    ...
                    } elseif (Mage::getStoreConfig('wsu_seo/seosuite/cross_domain')) {
                        $crossDomainStore = Mage::getStoreConfig('wsu_seo/seosuite/cross_domain');
                    }                    
        ...
        if (!$entity || $entity=='tag') {
            $productTags = Mage::getStoreConfigFlag('wsu_seo/google_sitemap/product_tags');
            if ($productTags) {
                $changefreq = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/product_tags_changefreq');
                $priority = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/product_tags_priority');
        ...
        if (!$entity || $entity=='cms') {
            $changefreq = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/page_changefreq');
            $priority = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/page_priority');
            
            $collection = Mage::getResourceModel('sitemaper/cms_page')->getCollection($storeId);
            foreach ($collection as $item) {
                $xml = sprintf('<url><loc>%s</loc><lastmod>%s</lastmod><changefreq>%s</changefreq><priority>%.1f</priority></url>',
                                htmlspecialchars($this->_trailingSlash($baseUrl . $item->getUrl())),
                                $date,
        ...
        if (!$entity || $entity=='additional_links') {
            ...
            $changefreq = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/link_changefreq');
            $priority = (string) Mage::getStoreConfig('wsu_seo/google_sitemap/link_priority');
            $addLinks = array_filter(preg_split('/\r?\n/', Mage::getStoreConfig(Wsu_Sitemaper_Block_Links::XML_PATH_ADD_LINKS, $storeId)));
            if (count($addLinks)) {
            ...
            $xml = Mage::getStoreConfig(Wsu_Sitemaper_Block_Links::XML_PATH_ADD_LINKS, $storeId);
            try {
        ...
        if (!$entity || $entity=='sitemap_finish') Mage::dispatchEvent('sitemaper_sitemap_generate_after', array('io_sitemap' => $io));
    ...
    protected function _trailingSlash($url) {
        if (Mage::getStoreConfigFlag('wsu_seo/seosuite/trailing_slash') && substr($url, -1)!='/' && !in_array(substr(strrchr($url, '.'), 1), array('rss', 'html', 'htm', 'xml', 'php'))) {
            $url.= '/';
        }   
        return $url;
    }
    ...
    protected function _openXml($io, $append = false) {
        if ($io->fileExists($this->_getSitemapFilename()) && !$io->isWriteable($this->_getSitemapFilename())) {
            Mage::throwException(Mage::helper('sitemaper')->__('File "%s" cannot be saved. Please, make sure the directory "%s" is writeable by web server.', $this->_getSitemapFilename(), $this->getPath()));
        }
    ...
    protected function _generateSitemapIndex($io) {
        ...
        if ($io->fileExists($this->getSitemapFilename()) && !$io->isWriteable($this->getSitemapFilename())) {
            Mage::throwException(Mage::helper('sitemaper')->__('File "%s" cannot be saved. Please, make sure the directory "%s" is writeable by web server.', $this->getSitemapFilename(), $this->getPath()));
        }
